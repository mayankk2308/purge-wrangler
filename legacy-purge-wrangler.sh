...
# Operation to perform ["" "tb-patch" "nv-patch" "uninstall" "recover" "check-patch" "version" "help"]
operation="$1"
...
usage()
{
  ...
  ${bold}Usage${normal}:
    ...
    \t${underline}${bold}No arguments${normal}: Same as tb-patch - standard Apple eGPU support.

    \t${underline}${bold}tb-patch${normal}: Patch AppleGPUWrangler for TB1/2 devices.

    \t${underline}${bold}nv-patch${normal}: Apply patch for NVDA eGPUs. Also patches TB1/2 devices.
...
# Option handlers
if [[ "$operation" == "" || "$operation" == "tb-patch" || "$operation" == "nv-patch" ]]
then
  check_sys_iotbswitchtype
  check_patch
  check_legacy_script_install
